 ...
 * <td valign=top>(the number of elitist individuals for subpopulation
 * <i>i</i>)</td>
 * </tr>
 * <tr>
 * <td valign=top><tt><i>base</i>.reduce-by.<i>i</i></tt><br>
 * <font size=-1>int >= 0 (default=0)</font></td>
 * <td valign=top>(how many to reduce subpopulation <i>i</i> by each
 * generation)</td>
 * </tr>
 * <tr>
 * <td valign=top><tt><i>base</i>.minimum-size.<i>i</i></tt><br>
 * <font size=-1>int >= 2 (default=2)</font></td>
 * <td valign=top>(the minimum size for subpopulation <i>i</i> regardless of
 * reduction)</td>
 * </tr>
 * <tr>
...
public class XGABreeder extends Breeder
{
	...
	public void setup(final EvolutionState state, final Parameter base)
	{
		...
		int size = state.parameters.getInt(p, null, 1); // if size is wrong, we'll let Population complain about it --
														// for us, we'll just make 0-sized arrays and drop out.
	...
	/**
	 * Elites are often stored in the top part of the subpopulation; this function
	 * returns what part of the subpopulation contains individuals to replace with
	 * newly-bred ones (up to but not including the elites).
	 */
	public int computeSubpopulationLength(EvolutionState state, Population newpop, int subpopulation, int threadnum)
	{
		if (!shouldBreedSubpop(state, subpopulation, threadnum)) { return newpop.subpops[subpopulation].individuals.length; // we're not breeding the population, just copy
		}
		// over the whole thing
		return newpop.subpops[subpopulation].individuals.length - numElites(state, subpopulation); // we're breeding
																									// population, so
																									// elitism may have
																									// happened
	}
	...
	public Population breedPopulation(EvolutionState state)
	{
		...
		if (clonePipelineAndPopulation) newpop = (Population) state.population.emptyClone();
		else
		{
			if (backupPopulation == null) backupPopulation = (Population) state.population.emptyClone();
		...
		for (int i = 0; i < state.population.subpops.length; i++)
		{
			if (reduceBy[i] > 0)
			{
				int prospectiveSize = Math.max(Math.max(state.population.subpops[i].individuals.length - reduceBy[i], minimumSize[i]), numElites(state, i));
		...
		// how many threads do we really need? No more than the maximum number of
		...
		if (numThreads < state.breedthreads) state.output.warnOnce("Largest subpopulation size (" + numThreads + ") is smaller than number of breedthreads (" + state.breedthreads + "), so fewer breedthreads will be created.");
		...
		for (int x = 0; x < state.population.subpops.length; x++)
		{
			...
			for (int y = 0; y < numThreads; y++)
			{
				...
				else numinds[y][x] = individualsPerThread;
		 ...
		 * x=0;x<state.population.subpops.length;x++) { // the number of individuals we
		 * need to breed int length = computeSubpopulationLength(state, newpop, x, 0);
		 * // the size of each breeding chunk except the last one int
		 * firstBreedChunkSizes = length/state.breedthreads; // the size of the last
		 * breeding chunk int lastBreedChunkSize = firstBreedChunkSizes + length -
		 * firstBreedChunkSizes * (state.breedthreads);
		 * 
		 * // figure numinds if (y < state.breedthreads-1) // not the last one
		 * numinds[y][x] = firstBreedChunkSizes; else // the last one numinds[y][x] =
		 * lastBreedChunkSize;
		 * 
		...
		else
		{
			 ...
			 * 
			 * // start up the threads for(int y=0;y<numThreads;y++) { SimpleBreederThread r
			 * = new SimpleBreederThread(); r.threadnum = y; r.newpop = newpop; r.numinds =
			 * numinds[y]; r.from = from[y]; r.me = this; r.state = state; t[y] = new
			 * Thread(r); t[y].start(); }
			 * 
			 * // gather the threads for(int y=0;y<numThreads;y++) try { t[y].join(); }
			 * catch(InterruptedException e) { state.output.
	...
	/**
	 * Returns true if we're doing sequential breeding and it's the subpopulation's
	 * turn (round robin, one subpopulation per generation).
	 */
	 ...
	 * declared public (for the benefit of a private helper class in this file), you
	 * should not call it.
	 */
	...
	protected void breedPopChunk(Population newpop, EvolutionState state, int[] numinds, int[] from, int threadnum)
	{
		for (int subpop = 0; subpop < newpop.subpops.length; subpop++)
		{
			...
			{
				// instead of breeding, we should just copy forward this subpopulation. We'll
				// copy the part we're assigned
			...
			else
			{
				...
				if (clonePipelineAndPopulation) bp = (BreedingPipeline) newpop.subpops[subpop].species.pipe_prototype.clone();
				else bp = (BreedingPipeline) newpop.subpops[subpop].species.pipe_prototype;

				// check to make sure that the breeding pipeline produces
				// the right kind of individuals. Don't want a mistake there! :-)
				int x;
				if (!bp.produces(state, newpop, subpop, threadnum)) state.output.fatal("The Breeding Pipeline of subpopulation " + subpop + " does not produce individuals of the expected species " + newpop.subpops[subpop].species.getClass().getName() + " or fitness " + newpop.subpops[subpop].species.f_prototype);
	...
	protected void unmarkElitesEvaluated(EvolutionState state, Population newpop)
	{
		...
		{
			if (!shouldBreedSubpop(state, sub, 0)) continue;
			for (int e = 0; e < numElites(state, sub); e++)
			{
				int len = newpop.subpops[sub].individuals.length;
				if (reevaluateElites[sub]) newpop.subpops[sub].individuals[len - e - 1].evaluated = false;
			}
		}
	}
	...
	protected void loadElites(EvolutionState state, Population newpop)
	{
		...
		for (int x = 0; x < state.population.subpops.length; x++)
		{
			if (numElites(state, x) > state.population.subpops[x].individuals.length) state.output.error("The number of elites for subpopulation " + x + " exceeds the actual size of the subpopulation", new Parameter(EvolutionState.P_BREEDER).push(P_ELITE).push("" + x));
			if (numElites(state, x) == state.population.subpops[x].individuals.length) state.output.warning("The number of elites for subpopulation " + x + " is the actual size of the subpopulation", new Parameter(EvolutionState.P_BREEDER).push(P_ELITE).push("" + x));
		}
		state.output.exitIfErrors();
		...
		for (int sub = 0; sub < state.population.subpops.length; sub++)
		{
			...
			if (numElites(state, sub) == 1)
			{
				...
				for (int x = 1; x < oldinds.length; x++)
					if (oldinds[x].fitness.betterThan(oldinds[best].fitness)) best = x;
