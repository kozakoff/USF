...
public class XGAEvaluator extends Evaluator
{
	...
	public void setup(final EvolutionState state, final Parameter base)
	{
		...
		else if (numTests > 1)
		{
			...
			if (m == null) state.output.warning("Merge method not provided to SimpleEvaluator.  Assuming 'mean'");
			else if (m.equals(V_MEAN)) mergeForm = MERGE_MEAN;
			else if (m.equals(V_MEDIAN)) mergeForm = MERGE_MEDIAN;
			else if (m.equals(V_BEST)) mergeForm = MERGE_BEST;
			else state.output.fatal("Bad merge method: " + m, base.push(P_NUM_TESTS), null);
		}
	...
	void expand(EvolutionState state)
	{
		...
		for (int i = 0; i < pop.subpops.length; i++)
		{
			...
			for (int j = 0; j < state.population.subpops[i].individuals.length; j++)
			{
				...
				{
					pop.subpops[i].individuals[numTests * j + k] = (Individual) (state.population.subpops[i].individuals[j].clone());
				}
			}
		}
	...
	public void evaluatePopulation(final EvolutionState state)
	{
		...
		if (state.evalthreads == 1)
		{
			...
			if (cloneProblem) prob = (SimpleProblemForm) (p_problem.clone());
			else prob = (SimpleProblemForm) (p_problem); // just use the prototype
	...
	public String runComplete(final EvolutionState state)
	{
		for (int x = 0; x < state.population.subpops.length; x++)
			for (int y = 0; y < state.population.subpops[x].individuals.length; y++)
				if (state.population.subpops[x].individuals[y].fitness.isIdealFitness()) return "Individual " + y + " of subpopulation " + x + " has an ideal fitness.";
	...
	class SimpleEvaluatorThread implements Runnable
	{
		...
		public void run()
		{
			...
			while (true)
			{
				...
				if (count >= subpops[subpop].individuals.length - start) count = subpops[subpop].individuals.length - start;
