...
/**
 * Several standard Evolutionary Computation functions are implemented. As the
 * SimpleFitness is used for maximization problems, the mapping f(x) --> -f(x)
 * is used to transform the problems into maximization ones.
 * 
 * <p>
 * <b>Parameters</b><br>
 * <table>
 ...
 * <font size=-1>String, one of: rosenbrock rastrigin sphere step noisy-quartic
 * kdj-f1 kdj-f2 kdj-f3 kdj-f4 booth griewank median sum product schwefel min
 * rotated-rastrigin rotated-schwefel rotated-griewank langerman lennard-jones
 * lunacek</font></td>
 * <td valign=top>(The vector problem to test against. Some of the types are
 * synonyms: kdj-f1 = sphere, kdj-f2 = rosenbrock, kdj-f3 = step, kdj-f4 =
 * noisy-quartic. "kdj" stands for "Ken DeJong", and the numbers are the
 * problems in his test suite)</td>
 * </tr>
 * <tr>
 * <td valign=top><i>base</i>.<tt>seed</tt><br>
 * <font size=-1>int > 0</font></td>
 * <td valign=top>(Random number seed for rotated problems)</td>
 * </tr>
 * </table>
 * 
 */

public class XGAECSuite extends Problem implements SimpleProblemForm {
	...
	public static final String problemName[] = new String[] { V_ROSENBROCK, V_RASTRIGIN, V_SPHERE, V_STEP,
			V_NOISY_QUARTIC, V_BOOTH, V_GRIEWANK, V_MEDIAN, V_SUM, V_PRODUCT, V_SCHWEFEL, V_MIN, V_ROTATED_RASTRIGIN,
			V_ROTATED_SCHWEFEL, V_ROTATED_GRIEWANK, V_LANGERMAN, V_LENNARDJONES, V_LUNACEK };

	public static final double minRange[] = new double[] { -2.048, // rosenbrock
			-5.12, // rastrigin
			-5.12, // sphere
			-5.12, // step
			-1.28, // noisy quartic
	...
	public static final double maxRange[] = new double[] { 2.048, // rosenbrock
			5.12, // rastrigin
			5.12, // sphere
			5.12, // step
			1.28, // noisy quartic
	...
	public void checkRange(EvolutionState state, int problem, double[] genome) {
		if (alreadyChecked || state.generation > 0)
			return;
		alreadyChecked = true;
		...
		for (int i = 0; i < state.population.subpops.length; i++) {
			...
			for (int k = 0; k < genome.length; k++) {
				if (species.minGene(k) != minRange[problem] || species.maxGene(k) != maxRange[problem]) {
					state.output.warning("Gene range is nonstandard for problem " + problemName[problem]
							+ ".\nFirst occurrence: Subpopulation " + i + " Gene " + k + " range was ["
							+ species.minGene(k) + ", " + species.maxGene(k) + "], expected [" + minRange[problem]
							+ ", " + maxRange[problem] + "]");
					return; // done here
				}
			}
		...
		if (problemType == PROB_LANGERMAN) {
			...
			if (genome.length > 10)
				state.output.fatal(
						"The Langerman function requires that the genome size be a value from 1 to 10 inclusive.  It is presently "
								+ genome.length);
		}
			...
			if (genome.length % 3 != 0)
				state.output.fatal(
						"The Lennard-Jones function requires that the genome size be a multiple of 3.  It is presently "
								+ genome.length);
		}
	...
	public void setup(final EvolutionState state, final Parameter base) {
		...
		if (wp.compareTo(V_ROSENBROCK) == 0 || wp.compareTo(V_F2) == 0)
			...
			state.output.warning("Incorrect parameter name (\"griewangk\") used, should be \"griewank\"",
					base.push(P_WHICH_PROBLEM), null);
			problemType = PROB_GRIEWANK;
	...
	public void evaluate(final EvolutionState state, final Individual ind, final int subpopulation,
		...
		// this curious break-out makes it easy to use the isOptimal() and function()
		// methods
		...
		if (fit < (0.0 - Double.MAX_VALUE)) // uh oh -- can be caused by Product for example
		{
			...
			state.output.warnOnce(
					"'Product' type used: some fitnesses are negative infinity, setting to lowest legal negative number.");
	...
	public double function(EvolutionState state, int function, double[] genome, int threadnum) {
		...
		case PROB_STEP:
			for (int i = 0; i < len; i++) {
				double gi = genome[i];
				// The reason for the 6 is that this is the equation De Jong used in the De Jong
				// Test Suite
		...
		case PROB_BOOTH:
			...
			double g0 = genome[0];
			double g1 = genome[1];
			value = (g0 + 2 * g1 - 7) * (g0 + 2 * g1 - 7) + (2 * g0 + g1 - 5) * (2 * g0 + g1 - 5);
			return -value;
		...
		case PROB_MEDIAN: // FIXME, need to do a better median-finding algorithm, such as
	...
	private double[][] afox10 = { { 9.681, 0.667, 4.783, 9.095, 3.517, 9.325, 6.544, 0.211, 5.122, 2.020 },
	...
	private double[] cfox10 = { 0.806, 0.517, 1.5, 0.908, 0.965, 0.669, 0.524, 0.902, 0.531, 0.876, 0.462, 0.491, 0.463,
			0.714, 0.352, 0.869, 0.813, 0.811, 0.828, 0.964, 0.789, 0.360, 0.369, 0.992, 0.332, 0.817, 0.632, 0.883,
			0.608, 0.326 };
	 ...
	 * ----------------- Rotation facility -----------------
	 * 
	 * This code is just used by the Rotated Schwefel and Rotated Rastrigin
	 * functions to rotate their functions by a certain amount. The code is largely
	 * based on the rotation scheme described in
	 * "Completely Derandomized Self-Adaptation in Evolutionary Strategies",
	 * Nikolaus Hansen and Andreas Ostermeier, Evolutionary Computation 9(2):
	 * 159--195.
	 * 
	 * We fix a hard-coded rotation matrix which is the same for all problems, in
	 * order to guarantee correctness in gathering results over multiple jobs. But
	 * you can change that easily if you like.
	 * 
	 */

	public static double[][][] rotationMatrix = new double[1][][]; // the actual matrix is stored in rotationMatrix[0]
																	// -- a hack
	...
	/**
	 * Multiply a column vector against a matrix[row][column]. Does not modify the
	...
	public static double[ /* row */ ][ /* column */] buildRotationMatrix(EvolutionState state, long rotationSeed,
			int N) {
		if (rotationSeed == ROTATION_SEED)
			state.output.warnOnce("Default rotation seed being used (" + rotationSeed + ")");

		MersenneTwisterFast rand = new MersenneTwisterFast(rotationSeed); // it's rare to need to do this, but we need
																			// to guarantee the same rotation space
