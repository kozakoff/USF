...
public class XGAMetamaskPipeline extends BreedingPipeline 
{
	private static final long serialVersionUID = 1L;
	public static final String P_METAMASK = "metamask";
    public static final int NUM_SOURCES = 1;
    
	@Override
	public Parameter defaultBase() { return VectorDefaults.base().push(P_METAMASK);	}
	...
	public int produce(final int min, final int max, final int start, final int subpopulation, final Individual[] inds, final EvolutionState state, final int thread) 
	{
		...
		if(thisState.resetMetas)
		{
			//reset meta genes
			for (int q = start; q < n + start; q++) 
			{
				((HCIndividual) inds[q]).resetMetas(state, thread);
